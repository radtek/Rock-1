...
namespace Rock.Communication
{
    ...
    {
        private static string _emailAddressRegex = @"\s*(\w+(?:[-+.]\w+)*@\w+(?:[-.]\w+)*\.\w+(?:[-.]\w+)*)\s*";
        private static string _lavaVariableRegex = @"({{\s*[^}}]+\s*}})";
        private static string _lavaTagRegex = @"({%\s*[^%}]+\s*%})";
        private static string _lavaBlockRegex = @"(({%\s*(?<tagName>(\w+)\s*).*%})).*({%\s*end\k<tagName>\s*%})";
        private static string _lavaShortcodeRegex = @"({\[\s*[^\]}]+\s*\]})";
        ...
        /// <returns></returns>
        public static EmailFieldValidationResultSpecifier Validate( string content, bool allowMultipleAddresses = false, bool allowLava = false )
        {
        ...
        /// <summary>
        /// Get a regular expression to validate an email address field with the specified settings.
        /// </summary>
        /// <param name="allowMultipleAddresses"></param>
        /// <param name="allowLava"></param>
        /// <returns></returns>
        public static string GetRegularExpression( bool allowMultipleAddresses = false, bool allowLava = false )
        {
            var validator = new EmailAddressFieldValidator { MultipleAddressesAreAllowed = allowMultipleAddresses, LavaIsAllowed = allowLava };
        ...
        public string GetRegularExpression()
        {
            ...
            if ( _allowMultipleAddresses )
            {
                if ( _allowLava )
                {
                    ...
                    regExString = $@"(({_lavaBlockRegex}|{_lavaTagRegex}|{_lavaShortcodeRegex}|{_lavaVariableRegex})(({_emailAddressRegex})([\s,])*)*)";
                }
                else
                {

                    // One or more email addresses, separated by whitespace or commas.
                    regExString = $@"(({_emailAddressRegex}[\s,]*)*)";
                }
            }
            else
            {
                if ( _allowLava )
                {
                    ...
                    regExString = $@"^\s*(({_lavaBlockRegex}|{_lavaTagRegex}|{_lavaShortcodeRegex}|{_lavaVariableRegex}|{_emailAddressRegex})*)\s*$";
                }
                else
                {
                    // A single email address.
                    regExString = $@"^{_emailAddressRegex}$";
                }
            }
